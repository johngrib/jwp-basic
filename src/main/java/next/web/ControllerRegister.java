...
import next.controller.Forward;
...
public class ControllerRegister {
    ...
    private static final Controller FORWARD = new Forward();
    ...
    private static Map<String, Controller> collectController() {
        ...
        final List<Class<?>> controllerList = ReflectionUtil.getAnnotatedClasses(classes, annotation.Controller.class);

        final Map<String, Controller> controllers = new HashMap<>();

        controllerList.stream().forEach(c -> {
            final String name = c.getAnnotation(annotation.Controller.class).name();
